...
def createModelMenu(self, modelMenu):
    ...
    modelMenu.add_command(label="new entity0", command=lambda x=self: x.newModesentity0(x) )
def addButtons(self, createNewASGroot = 0):
    mmToolBar = Frame(self.toolBar, relief = RAISED)
    b = Label(mmToolBar, text = "ERModel", font = ("Helvetica",8), relief = GROOVE)
    b.pack(side = TOP, fill = X, ipady = 5)
    self.openMetaModels.newItem(ATOM3String("ERModel"))
    auxPanel = Frame(mmToolBar, relief = GROOVE)
    newentity0=Button(auxPanel, text="new entity0", command=lambda x=self: x.newModeentity0(x) )
    newentity0.pack(side=LEFT, padx=2, pady=2)

    auxPanel.pack(side=TOP, fill=X, ipady=2)
    if createNewASGroot:
       self.ASGroot = ASG_ERModel(self)
    self.buttonList.append((mmToolBar, "ERModel"))
    return mmToolBar
def configureUserActions(self):
    ...
    self.userActionsMap[self.NEWentity0] = self.createNewentity0
...
def setConnectivity(self):
    self.ConnectivityMap['entity0']={
           'entity0': [] }
    
    self.CardinalityTable['entity0']={
          'entity0': None }
    
    self.entitiesInMetaModel['ERModel']=[entity0]

    
def newModeentity0(self):
   self.mode = self.NEWentity0

def createNewentity0(self, wherex, wherey, screenCoordinates = 1):
   self.fromClass = None
   self.toClass = None
   ...
   new_semantic_obj = entity0(self)
   ne = len(self.ASGroot.listNodes["entity0"])
   if new_semantic_obj.keyword_:
      new_semantic_obj.keyword_.setValue(new_semantic_obj.keyword_.toString()+str(ne))
   if screenCoordinates:
      new_obj = graph_entity0(self.UMLmodel.canvasx(wherex), self.UMLmodel.canvasy(wherey), new_semantic_obj)
   else: # already in canvas coordinates
      new_obj = graph_entity0(wherex, wherey, new_semantic_obj)
   new_obj.DrawObject(self.UMLmodel, self.editGGLabel)
   self.UMLmodel.addtag_withtag("entity0", new_obj.tag)
