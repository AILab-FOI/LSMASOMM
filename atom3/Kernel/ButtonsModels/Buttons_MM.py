...
def createModelMenu(self, modelMenu):
    ...
    modelMenu.add_command(label="new ButtonConfig", command=lambda x=self: x.newModesButtonConfig(x) )
def setConnectivity(self):
    self.ConnectivityMap['ButtonConfig']={
           'ButtonConfig': [] }
    
    self.CardinalityTable['ButtonConfig']={
          'ButtonConfig': [] }
    
    self.entitiesInMetaModel['Buttons']=["ButtonConfig"]

    
def createNewButtonConfig(self, wherex, wherey, screenCoordinates = 1):
   self.fromClass = None
   self.toClass = None
   ...
   new_semantic_obj = ButtonConfig(self)
   ne = len(self.ASGroot.listNodes["ButtonConfig"])
   if new_semantic_obj.keyword_:
      new_semantic_obj.keyword_.setValue(new_semantic_obj.keyword_.toString()+str(ne))
   if screenCoordinates:
      new_obj = graph_ButtonConfig(self.UMLmodel.canvasx(wherex), self.UMLmodel.canvasy(wherey), new_semantic_obj)
   else: # already in canvas coordinates
      new_obj = graph_ButtonConfig(wherex, wherey, new_semantic_obj)
...
def createNew_Model(self, wherex, wherey, screenCoordinates = 1):
   self.toClass = None
   self.fromClass = None
   new_semantic_obj = ASG_Buttons(self)
   ne = len(self.ASGroot.listNodes["ASG_Buttons"])
   if new_semantic_obj.keyword_:
      new_semantic_obj.keyword_.setValue(new_semantic_obj.keyword_.toString()+str(ne))
   if screenCoordinates:
   ...
   new_obj.DrawObject(self.UMLmodel, self.editGGLabel)
   self.UMLmodel.addtag_withtag("ASG_Buttons", new_obj.tag)
   new_semantic_obj.graphObject_ = new_obj
   self.ASGroot.addNode(new_semantic_obj)
   self.mode=self.IDLEMODE
   if self.editGGLabel :
...
def fillTypesInformation(self):
    ...
    obj = ATOM3TypeInfo(self)
    params  = []
    obj.setValue(("ImageText", "ImageText", params, (None, 1) ))
    objs.append(obj)
