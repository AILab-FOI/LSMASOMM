# __ File: ATOM3Action.py __________________________________________________________________________________________________
#  Implements  : class ATOM3Action
#  Author      : Juan de Lara
#  Description : A class for the ATOM3 Actions type. Just like Constraints...
#  Modified    : Feb 26, 2005
#  Changes :
...
class ATOM3Action(ATOM3Type):

   def __init__(self, attribs = None, constrName = None, prefix="self.", kind = ['PREaction', 'POSTaction'],
   ...
   def clone (self):
      ...
      cloneObject = ATOM3Action(self.classAttribs, None, self.prefix, kinds, acts)			# call constructor for clone object
   ...
   def setValue(self, value):
      ...
      if type(value) == NoneType:                                       # INPUT = None. Valid (to set to None the object)
         ...
         raise ATOM3BadAssignmentValue, "ATOM3Action: Bad type in setValue()"
      ...
      if type(language) == TupleType:					# May be is a tuple ([languages...], selected)
         ...
         raise ATOM3BadAssignmentValue, "ATOM3Action: Bad type in setValue() (2nd argument in tuple)"
        ...
        if type(kind) == TupleType and len(kind) == 2:
           if not self.kind: self.kind = ATOM3Enum(kind[0], kind[1])
              ...
              try:
                 ...
                 raise ATOM3BadAssignmentValue, "ATOM3Action: Bad type in setValue(), "+ str(kind)
        elif type(kind) != NoneType:
           raise ATOM3BadAssignmentValue, "ATOM3Action: Bad type in setValue() (3rd argument in tuple)"
      ...
      if actions:
        if type(actions) == TupleType and len(actions) == 2:
           ...
           raise ATOM3BadAssignmentValue, "ATOM3Action: Bad type in setValue() (4th argument in tuple)"
      ...
      if type(code) == StringType or type(code) == NoneType:
         ...
         raise ATOM3BadAssignmentValue, "ATOM3Action: Bad type in setValue() (5th argument in tuple)"
   ...
   def show (self, master, topWindowParent = None):
      ...
      Label(nameFrame, text="Action name: ", bg='light blue',
      ...
      # create a pop-up menu with the UML class attributes...
      self.constCodeWidget.bind("<Button-3>", self.showPopup)	
      
      self.textWidget = self.constCodeWidget
      self.textWidget.bind("<Delete>", lambda e=None, s=self: processDelete(s))
      self.textWidget.bind("<BackSpace>", 
                           lambda e=None, s=self: processBackspace(s) )  
   ...
   def writeConstructor2File(self, file, indent, objName='at', depth = 0, generatingCode = 0):
      ...
      if self.constCodeWidget:					# if the code widget is visible, update the attribute value
         self.constCode = self.constCodeWidget.get(1.0, END)
      file.write(indent+objName+"=ATOM3Action()\n")
      if self.isNone():
