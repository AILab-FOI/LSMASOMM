# __oval.py_____________________________________________________
from ASGNode import *
...
from graph_oval import *
class oval(ASGNode, ATOM3Type):

   def __init__(self, parent = None):
      ASGNode.__init__(self)
      ATOM3Type.__init__(self)
      self.graphClass_ = graph_oval
      self.parent = parent
   ...
   def toString(self, maxWide = None, maxLines = None ):
      rs = self.fill.toString()+' '+self.stipple.toString()+' '+self.outline.toString()+' '+self.width.toString()
      if maxWide: return self.strValue[0:maxWide-3]+'...'
      else: return rs


   def getValue(self):
      return (self.fill.getValue(),self.stipple.getValue(),self.outline.getValue(),self.width.getValue(),)
   ...
   def invalid(self):
      ...
      return self.fill.invalid() or self.stipple.invalid() or self.outline.invalid() or self.width.invalid() 

   def clone(self):
      cloneObject = oval( self.parent )
