...
######Change LOG: 
"""By Raphael Mannadiar (2009.10.10)
Changed propagateEdge2Subclasses(superclass) to propagateEdge2Subclasses(superclass, associationList). 
This function is used to propagate associations from superclasses to subclasses during code generation. 
It worked by building associationList by inspecting in/out_connections. However, only the superclass 
actually has these conections in the class diagram. Since the propagation function doesn't create these 
connections for subclasses but merely simulates that they're there, when propagateEdge2Subclasses() was 
recursively called on a subclass, no inherited connections were detected causing associations to only 
propagate one level down. With the added 2nd parameter, associationList grows as the function is called 
recursively so that associations reach the bottom level.
"""
...
def propagateCardinalities(self):
  ...
  for superclass in superClassList:
    propagateEdge2Subclasses(superclass, [])						######Change: RM 2009.10.10 (see log above)
    
  
  
def propagateEdge2Subclasses(superclass, associationList):	######Change: RM 2009.10.10 (see log above)
  """
  ...
  #associationList = []													######Change: RM 2009.10.10 (see log above)
  ...
  for link in superclass.in_connections_: 
    if(link.__class__.__name__ == Magic().inheritanceClassName):
      for subclass in link.in_connections_:
        ...
        propagateEdge2Subclasses(subclass, associationList)		######Change: RM 2009.10.10 (see log above)
